...
class Attention(nn.Module):
    def __init__(
        self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0, st_mode="vanilla"
    ):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_s = MLP(
            in_features=dim, hidden_features=mlp_hidden_dim, out_features=mlp_out_dim, act_layer=act_layer, drop=drop
        )
        self.mlp_t = MLP(
            in_features=dim, hidden_features=mlp_hidden_dim, out_features=mlp_out_dim, act_layer=act_layer, drop=drop
        )
...
class DSTformer(nn.Module):
    ...
    def forward(self, x, return_rep=True): #, only_classify=False):
        # if only_classify:
        #     x = self.head(x)
        #     return x
        ...
        if return_rep:
            return x
        keypoints = self.head(x)
        return x, keypoints # representation, keypoints
